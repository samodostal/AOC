...
void solve(vector<string> path, vector<vector<string>> instructions, int &r, string lucky_small) {
  ...
  FOR(i, instructions) {
    if (instructions[i][0] == last_path) {
      ...
      if(possible_next == "start") continue;

      if (!is_capital(possible_next)) {
        if(path_contains(path, possible_next) == 1) {
          if(empty(lucky_small)) {
            auto path_copy = path;
            path_copy.push_back(possible_next);
            solve(path_copy, instructions, r, possible_next);
            continue;
          } else {
            continue;
          }
      ...
      auto path_copy = path;
      path_copy.push_back(possible_next);
      solve(path_copy, instructions, r, lucky_small);
    }
  }
}
